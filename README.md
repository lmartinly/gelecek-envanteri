...
1. GitHub'da yeni repo aç (ör. `gelecek-envanteri`), bu klasördeki **6 dosyayı** köke yükle:
...
## Sürüm 1.2.0 — Paylaşılabilir kurulum

Uygulama artık **verisi olmayan bir cihazda** bir kurulum ekranıyla açılır: Claude'a götürülecek hazır bir prompt verir, dönen JSON listesini alıp odaları/kategorileri/ürünleri kurar. Üç yol var: Claude listesi yapıştır (ya da dosya seç; uygulama yedeği de kabul edilir), hazır şablonla başla (156 ürünlük çeyiz şablonu), ya da boş başla. **Verisi olan cihaz bu ekrana asla girmez** — mevcut envanterin etkilenmez.

Ayarlar → **Claude ile liste ekle**: aynı promptla üretilmiş bir listeyi mevcut envanterine ekler. Yalnızca ekler: aynı isimli oda ve kategoriler eşleştirilir, o odada zaten olan ürünler atlanır, fiyat/durum/taksit bilgilerine ve hedef tarihine dokunulmaz.

**Arkadaşına paylaşmak için:** site linkini gönder, Safari'de açıp Ana Ekrana Ekle demesi yeterli. Kendi cihazında kurulum ekranını görecek, kendi listesini kuracak; veriler cihaz bazlı olduğu için senin envanterini görmez. Drive yedeği isterse kendi Client ID'sini Ayarlar'a girmesi gerekir (README adım 2).
